...
var swaps = {'<':'&lt;','>':'&gt;','"':'&#34',"'":'&#39'};
var finalSanitizers = {'<b>':'#b@', '</b>':'#/b@', '<b/>':'#/b@', '<i>':'#i@', '</i>':'#/i@', '<i/>':'#/i@', '<':'&lt;','>':'&gt;','"':'&#34',"'":'&#39',' ':'%20'};
var openingSanitize = {'<':'&lt;','>':'&gt;','"':'&#34',"'":'&#39','%20':' ', '%27': "'", '#b@':'<b>', '#/b@':'</b>', '#b/@':'</b>', '#i@':'<i>', '#/i@':'</i>', '#i/@':'</i>'};
...
function imp(key) {
    ...
    if (moredesc == 1) {
        document.getElementById('moreDesc').innerHTML = act;
    }
/*    for (i = 0; i < expectcount; i++) {
        var localoffset = 0;
        if (i >= 2 && i <= 29) {
            if (i >= 23) if(abi6 == 0) localoffset += 5;
            if (i >= 18) if(abi5 == 0) localoffset += 5;
            if (i >= 14) if(abi4 == 0) localoffset += 5;
            if (i >= 8) if(abi3 == 0) localoffset += 5;
            if (i >= 4) if(abi2 == 0) localoffset += 4;
